...
- [x] **Дизайн карточек каталога.** `ChartCard` переработан: иконка-плитка,
  ...
  с разделителем.
- [x] **Нейтральные цвета в «Изменениях» (CHANGELOG).** Группы
  `Added`/`Changed`/`Removed`/... выводятся нейтральными чипами (оттенки
  grayscale-семейств), без семантических зелёного/красного.

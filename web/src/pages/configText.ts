...
export const CONFIG_TEXT: Record<string, string> = {
  ...
  GITLAB_INSTANCE_DIR_TEMPLATE:
    "Как называется папка заказанного сервиса внутри репозитория, в папке своего кластера и namespace. Пусто - по имени сервиса. Заказ остаётся в своей папке, поэтому изменение действует только на новые заказы.",
  ...
  ARGOCD_APP_NAME_TEMPLATE:
    "Как строится имя приложения Argo CD для заказа. Заказ остаётся под своим именем, поэтому изменение действует только на новые заказы.",

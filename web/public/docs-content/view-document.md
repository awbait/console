...
Редактор подсказывает по ходу письма: Ctrl+Space открывает список ключей документа и полей этой версии чарта, наведение на ключ объясняет, зачем он нужен. Черновик сохраняется сам через пару секунд после правки, кнопка **Сохранить черновик** нужна, только если ждать не хочется. Кнопка **Взять из версии** над редактором подставляет документ другой версии этого же чарта: обычно новая версия отличается от прошлой парой полей, и начинать с чистого листа незачем.

## Написать документ с помощью нейросети

Документ можно не писать руками. Кнопка **Скачать промт** вверху этой страницы отдаёт файл с правилами формата, алгоритмом и примерами, по которому любая нейросеть соберёт документ для любого чарта. Кнопка **Скопировать промт** кладёт тот же текст в буфер обмена.

Что сделать:

1. Вставьте промт в диалог с нейросетью целиком.
2. Следом приложите `values.schema.json` чарта. Его текст есть во вкладке схемы в конструкторе. Если у чарта есть зависимости, приложите и `Chart.yaml`, и схемы зависимостей из вкладки «Зависимости».
3. Своими словами опишите, что заказчик должен заполнить при заказе, что сможет менять потом и что портал должен проставить сам.
4. Полученный документ вставьте в редактор конструктора. Конструктор сверит каждое имя и каждый указатель со схемой чарта и покажет ошибки, если нейросеть придумала поле или ошиблась в пути.

Предпросмотр справа стоит проверить глазами: нейросеть знает формат, но не знает вашего сервиса. Подписи полей, порядок и то, что спрятано, а что показано, решаете вы.

> [!TIP]
> Не отправляйте нейросети значения из реальных заказов. Для документа нужна только схема чарта: она описывает форму полей, а не их содержимое.
...
| `initial` | нет | значения, с которыми открывается форма нового заказа |
...
Поле чарта, который этот чарт тянет за собой, называется через косую черту: `"pooler/poolMode"`. Об этом ниже, в разделе «Поля зависимостей чарта».
...
Чарт может тянуть за собой другие чарты - в `Chart.yaml` они перечислены в блоке `dependencies`. Helm кладёт значения такого чарта в values родителя под одним ключом: под алиасом, если он задан, иначе под именем чарта. Портал показывает эти чарты во вкладке «Зависимости» рядом со схемой: там видно ключ, версию и схему зависимости.

Поле зависимости называется через косую черту: `<ключ>/<поле>`.

```json
{
  "views": {
    "order": {
      "include": ["naming", "pooler/poolMode", "pooler/maxClientConn"],
      "overrides": {
...
Так же пишутся `exclude`, `required` и ключи `overrides`. А там, где документ ждёт JSON pointer (`identity`, `namespace`, `defaults`, `initial`, `items`), путь до поля зависимости выглядит как любой другой: `/pooler/poolMode`.

Есть три правила, о которых стоит знать заранее.

**Поля зависимости не появляются в форме сами.** Внешний чарт описывает сотни полей, и форма заказа, открывшаяся со всеми, была бы бесполезна. Пока вы не назвали поле в `include`, его в форме нет. Имя ключа без косой черты (`"pooler"`) означает всю зависимость целиком.

**Схему зависимости показывает только сама зависимость.** У большинства внешних чартов `values.schema.json` нет вовсе, и тогда вкладка честно об этом говорит: описывать поля нечем, и назвать их в документе не получится.

**Схема чарта главнее там, где она что-то говорит.** Чарты обычно оставляют под значения сабчарта заглушку: заголовок, описание и `ui:widget: "hidden"`, без единого поля внутри. Заголовок и описание из чарта остаются, а поля приезжают из зависимости. Если же чарт описывает эти значения сам, со своими `properties`, портал берёт их из чарта и схему зависимости не подмешивает.

Портал разбирает один уровень зависимостей: зависимости зависимостей он не показывает.
...
**Поле элемента** - `path`, путь внутри элемента через `/`, без косой черты в начале. Сегмент `*` перебирает массив в этом месте и склеивает найденное.

```json
{ "path": "from/*/namespace", "label": "Неймспейс" }
```

Тот же `*` перебирает и значения словаря, если в этом месте лежит объект с произвольными ключами, например метки подов. Сегмент `*key` перебирает ключи такого словаря, `*val` - его значения, как `*`. Число после перебора берёт один элемент из собранного: `selector/*val/0` покажет значение первой метки.
...
Ключ - указатель на поле values, значение - строка, число или «да/нет». Записанное здесь **перезаписывает** то, что пришло из формы, поэтому раздел годится для полей, спрятанных от заказчика: пометок о происхождении, служебных констант.

Указатель умеет заходить в списки по номеру: `/gateways/0/ipAddress` пишет в тот шлюз, который в заказе есть. Новых элементов списка портал не создаёт: сколько их в заказе, решает заказчик, поэтому указатель на элемент, которого нет, ничего не запишет.
...
Ссылка всегда даёт текст, а поле в чарте не всегда текстовое. Портал приводит значение к тому, что объявлено в схеме чарта: в числовое поле уедет число, в переключатель - «да» или «нет». Привести не удалось, например в числовом поле оказались буквы, - портал не сохранит заказ и назовёт поле. Иначе заказчик получил бы жалобу на поле, которого он не заполнял.

Конструктор говорит об этом заранее, когда может: ссылка на команду или автора в числовом поле не сработает никогда, а переменная проверяется по тому значению, которое в ней сейчас.
...
## initial: с чем открывается форма

Значения, которые заказчик увидит в форме нового заказа уже заполненными. В отличие от `defaults`, поле остаётся видимым и его можно поправить: что человек в итоге оставил, то и сохранится.

```json
{ "initial": { "/contacts/responsible": "{{.User.Name}}" } }
```

Блок работает только при создании заказа. Заказ, который уже существует, портал не трогает: подставить значение поверх чужой правки означало бы вернуть то, от чего человек отказался.

Ссылки здесь те же, но доступны не все: форма ещё заполняется, поэтому она пока не знает ни неймспейса, ни кластера, ни имени сервиса. Остаются `{{.Team}}`, `{{.Chart}}`, `{{.ChartVersion}}`, `{{.User.Name}}`, `{{.User.Subject}}` и переменные платформы. Ссылку, на которую форма ответить не может, конструктор не пропустит.
...
Все блоки вместе на примере брокера очередей: заказчик заводит один брокер, потом добавляет к нему очереди во вкладке и правит ресурсы из меню.

Схема чарта, сокращённо: верхние поля `identity` (теги окружения, спрятаны чартом), `broker` (список из одного брокера с полями `name`, `resources`, `queues`) и `contacts` (поле `responsible`).

```json
{
  "views": {
    "order": {
      "$comment": "Заказ: имя брокера и ответственный. Теги окружения проставляет портал.",
      "identity": "/broker/0/name",
      "include": ["broker", "contacts"],
      "overrides": {
        "broker": {
          "title": "Брокер",
          "ui:widget": "single",
          "ui:view": {
            "exclude": ["queues", "resources"],
            "overrides": { "name": { "ui:readOnly": true } }
          }
        }
      }
    },
    "queue": {
      "$comment": "Форма одной очереди: описывает элемент списка queues, а не корень values.",
      "overrides": { "name": { "ui:readOnly": true } }
    },
    "resources": {
      "include": ["broker"],
  ...
  "tabs": [
    {
      "id": "queues",
      "title": "Очереди",
      "items": "/broker/0/queues",
      "form": "queue",
      "addLabel": "Добавить очередь",
      "ui:table": [
        { "path": "name", "label": "Имя" },
        { "path": "durable", "label": "Сохраняемая" }
      ]
    }
  ],
  "actions": [
...
- форма заказа показывает брокера как один блок, без списка очередей и ресурсов: очереди заводят потом, ресурсы правят из меню;
- имя брокера после создания не меняется: на него ссылаются очереди;
- view `queue` описывает одну очередь, поэтому её `overrides` называют поля элемента, а не корня;
- view `resources` показывает тот же массив `broker`, но оставляет в нём одно поле;
- теги окружения заказчик не видит: их проставляет `defaults`, а ответственного форма подставляет из автора заказа, и его можно поправить.

Больше примеров, в том числе с зависимостями чарта, вычисляемыми колонками и графом, в промте для нейросети: кнопка **Скачать промт** вверху страницы.
...
| `Поля "x" нет в схеме зависимости "y"` | зависимость есть, а такого поля в её схеме нет. Сверьтесь со вкладкой «Зависимости» |
| `Указатель "/a/b" не находит поле` | путь ведёт в никуда. Проверьте имена и индексы |
| `Путь "/a/b" не находит массив` | `items` вкладки или `from` правила `enums` указывает не на список |
...
| `View "order" это форма заказа` | `order` поставлена формой элемента вкладки или пунктом меню, а туда нужна другая view |
| `Не хватает view "order"` | нет формы заказа, а она обязательна |
| `Id "x" зарезервирован` | у вкладки `id` из занятых: `info`, `history`, `order` |
| `Вкладка с id "x" уже есть` | две вкладки с одним `id`, действия не смогут их различить |
| `Ключ "include" задаёт вложенную view` | ключ положен рядом с `title`, а нужен внутри `ui:view` |
| `Ключ "x" указан дважды` | один ключ два раза в одном объекте, JSON оставит только последний |
| `Нет такой ссылки` | в `defaults` или `initial` ссылка не из списка: опечатка или переменная, которой нет |
...
Черновик сохраняется сам и никому, кроме вас, не виден. Когда форма выглядит как надо, отправьте версию на согласование кнопкой **Отправить на согласование**: она доступна, пока под редактором нет ни одной ошибки. Порядок и статусы описаны в разделе [Публикация сервиса](publishing).

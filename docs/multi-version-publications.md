...
- [x] Сделано (миграция `000015_drop_legacy_publication_view.sql`): удалены
  ...
  (старшая orderable и рекомендуемая соответственно).
- [x] Сид больше не создаёт публикации: в новой инсталяции сидится только
  системная категория `uncategorized` (`store.SeedCategories`). Сервисы
  попадают в каталог через регистрацию либо автодискавери + усыновление.

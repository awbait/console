...
> **Статус upstream'ов:** архитектура строится вокруг портов (интерфейсов) + in-memory фейков, чтобы полный happy-path гонялся в тестах без реальных систем. Переключения в рантайме нет: запущенный портал всегда ходит в настоящие Harbor, GitLab и Argo CD. См. раздел «Тестирование».
...
- **Путь к манифестам:** `{GITLAB_GITOPS_GROUP}/{team-subgroup}/{chart}/{cluster}/{namespace}/{instance}/`.
  ...
  - `namespace` - namespace назначения заказа. Отдельным уровнем, потому что одна команда может заказать один сервис под одним именем в несколько namespace: это разные сервисы, и файлы у них разные. Уровень задаёт сам портал, а не шаблон, поэтому это выполняется при любом `GITLAB_INSTANCE_DIR_TEMPLATE`.
  - `instance` - по шаблону `GITLAB_INSTANCE_DIR_TEMPLATE`. Пусто - `service_name`. Доступны `.Team`, `.Chart`, `.ServiceName`, `.Namespace`, `.Cluster`. Шаблон обязан давать разным сервисам одной команды, одного чарта и одного namespace разные папки, иначе они пишут одни и те же файлы.
...
- При создании нового заказа с тем же `service_name` (для той же команды, того же чарта и того же namespace) - это **новый заказ с новым ID**, старый продолжает существовать в истории под пометкой "deleted"
...
- `GET /notifications` - лента читающего, новые сверху (`before`, `limit`)
- `GET /notifications/unread` - счётчик для колокольчика
- `GET /notifications/events` - SSE-сигнал «есть новое», без содержимого
- `POST /notifications/{id}/read` - отметить одно
- `POST /notifications/read-all` - отметить всё, что уже пришло

Кому адресовано уведомление, решается при чтении: в базе лежит правило
(`audience` = `user` | `team` | `role` | `all`), а команды и роль берутся из
сессии. Поэтому список получателей не устаревает, когда меняется состав команды.
Текст в базе не хранится: там `kind` и `payload`, фразу собирает интерфейс.
Прочитанное чистится через 90 дней, непрочитанное живёт, пока его не прочитают.
...
> **Несколько реплик.** Реплики договариваются через Redis (`CACHE=redis`), и обе половины держатся на нём.
>
> - **Шина событий** (`internal/events`): реплика раздаёт событие своим SSE-подписчикам сразу и параллельно кладёт его в канал `console:events`, откуда его забирают остальные. Событие несёт origin отправителя, поэтому назад к себе не возвращается. Доставка best-effort: пропущенное событие клиент добирает перечитыванием состояния на следующем подключении.
> - **Leader election** (`internal/leader`): аренда в ключе `console:leader` на 30 секунд с продлением каждые 10. Фоновые циклы (поллер реконсиляции, обновление gauge'ов заказов, отправка уведомлений о конфигурации) работают только у держателя аренды. Если Redis не отвечает, реплика держит аренду до конца её срока и затем снимает с себя лидерство: раньше этого момента ключ не может забрать никто другой, позже - проверить нечем.
> - Просьба «сверить сейчас» от webhook'а тоже идёт через шину: доставка приходит на случайную реплику, а разбирает её лидер.
> - По шине же расходятся два факта, которые иначе остались бы на одной реплике: приход webhook'а (счётчики читает страница конфигурации и кнопка проверки доставки, а доставка попадает на случайную реплику) и содержимое последнего токена входа (сессии общие, поэтому человек входит через одну реплику и работает на любой). Событие несёт отметку «своё или чужое», чтобы реплика не посчитала одну доставку дважды.
>
> С `CACHE=memory` шины между процессами нет и реплика всегда считает себя лидером, поэтому **больше одной реплики требует `CACHE=redis`**.
...
Каждый домен зависит от интерфейса, а не от конкретного клиента: `HarborPort`, `GitLabPort`, `ArgoCDPort`. Две реализации, но переключения в рантайме нет: бинарь всегда собирает real-клиенты, фейки подставляют тесты.
...
Фейки живут в тестах: полный happy-path заказа гоняется без реальных upstream'ов в `go test`. Запущенный портал так поднять нельзя - он требует адреса и токены Harbor, GitLab и Argo CD, иначе не стартует.
...
- Deployment (по умолчанию 1 реплика; больше одной работает при `CACHE=redis`, см. раздел про статусы)
- Service ClusterIP
- Ingress
- ServiceMonitor
- **HPA** - отдельная задача (#202)

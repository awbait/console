...
- `make stand-gitlab-webhooks` - завести GitLab -> портал MR-вебхук (для
  ...
  после заказа нового сервиса**, иначе его репо останется без вебхука (в режиме
  `hybrid` мерж всё равно подберёт поллер, в `webhook` - потеряется). Охват можно
  задать явно: `-Scope group|system|project`. Идемпотентно. Портал должен

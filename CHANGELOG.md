...
### Changed
- The order form says what a field takes: while you type, the requirements are
  listed beside it - which characters, what length, what range - with the ones
  already satisfied ticked off. Until now that had to be guessed, or learned
  from the error after sending the order.

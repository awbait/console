...
func TestVersionManagementAPI(t *testing.T) {
	...
	// Save a draft view for version 3.1.0.
	...
	}
	{
		...
		if len(pending) != 1 || pending[0].Version.ChartVersion != "3.1.0" {
			t.Fatalf("unexpected pending versions: %+v", pending)
		}
	}
	// A member cannot approve.
	if rec := do(devReq("POST", base+"/versions/3.1.0/approve", "core", nil)); rec.Code != http.StatusForbidden {
	...
	if rec := do(devReq("POST", base+"/versions/3.1.0/orderable", "core", map[string]any{"orderable": true})); rec.Code != http.StatusOK {
		t.Fatalf("orderable: %d body=%s", rec.Code, rec.Body.String())
	}
	if rec := do(devReq("POST", base+"/recommended", "core", map[string]any{"version": "3.1.0"})); rec.Code != http.StatusNoContent {
...
func TestCatalogAndViewExposeVersions(t *testing.T) {
	...
	view := json.RawMessage(`{"views":{"order":{"identity":"/auth/database","include":["auth"]}}}`)
	...
	p, err := srv.Pubs.Create(ctx, owner, publications.CreateInput{
		ChartProject: "platform", ChartName: "postgres", CategoryID: "network", OwnerTeam: "core",
	})
	if err != nil {
		t.Fatal(err)
	}
	for _, cv := range []string{"15.4.1", "15.4.2"} {
	...
	if err := srv.Pubs.SetRecommendedVersion(ctx, owner, p.ID, "15.4.1"); err != nil {
		t.Fatal(err)
	}
	...
	for _, c := range cat.Charts {
		if c.Project == "platform" && c.Name == "postgres" {
			found = true
			if c.Publication == nil || !c.Publication.Published {
				t.Fatalf("publication not published: %+v", c.Publication)
			}
			if c.Publication.RecommendedVersion != "15.4.1" {
				t.Fatalf("recommended want 15.4.1, got %q", c.Publication.RecommendedVersion)
			}
			// Highest first.
			got := c.Publication.OrderableVersions
			if len(got) != 2 || got[0] != "15.4.2" || got[1] != "15.4.1" {
				t.Fatalf("orderable versions want [15.4.2 15.4.1], got %v", got)
			}
		}
	}
	if !found {
	...
	rec = httptest.NewRecorder()
	h.ServeHTTP(rec, devReq("GET", "/api/v1/charts/platform/postgres/view?version=15.4.2", "core", nil))
	if rec.Code != http.StatusOK {
